...
const popUp = (id) => {
  ...
  for (let i = 0; i < projectArray.length; i += 1) {
    if (projectArray[i].ID === id) {
      ...
      for (let i = 0; i < modalTech.length; i += 1) {
      ...
      livetextSpan.className = 'span-live-text';
      livetextSpan.innerHTML = 'See Live';
      const liveimgSpan = document.createElement('span');
      liveimgSpan.className = 'span-live-image';
      const liveImg = document.createElement('img');
      liveImg.src = 'image/modal-dsktp-Icon-see-live.png';
      liveImg.alt = 'live icon';
      liveimgSpan.appendChild(liveImg);
...
const addClickEventToSeeproject = () => {
